...
const DATASET_CONFIG: Record<
  DatasetKey,
  { title: string; columns: ColumnDef[]; apiPath: string }
> = {
  floods: {
    title: "Data Genangan",
    apiPath: "floods",
...
export function DataTableModal({
  ...
  async function handleAdd() {
    ...
    try {
      ...
      if (datasetKey === "floods") await api.createFlood(payload as Parameters<typeof api.createFlood>[0]);
      else if (datasetKey === "depots") await api.createDepot(payload as Parameters<typeof api.createDepot>[0]);
      else if (datasetKey === "ifs") await api.createIF(payload as Parameters<typeof api.createIF>[0]);
      else await api.createFaskes(payload as Parameters<typeof api.createFaskes>[0]);
      setIsAdding(false);
      onReload();
    } catch (e) {
      alert(e instanceof Error ? e.message : "Gagal menambahkan");
    } finally {
      setBusy(false);
    }
  }
